...
<nav class="flex items-center justify-between px-4 sm:px-8 py-4 md:py-6 bg-white border-b border-slate-200 shadow-sm">
    ...
    <div class="flex items-center gap-3 md:gap-4">
        ...
        @if (Route::has('login'))
            @auth
                <a href="{{ url('/dashboard') }}" class="px-4 py-2.5 text-primary font-semibold border border-primary rounded-xl hover:bg-primary-pale transition-colors text-sm">{{ __('messages.nav_cabinet') }}</a>
            @else
                <a href="{{ route('login') }}" class="px-4 py-2.5 text-slate-600 font-semibold hover:text-primary transition-colors text-sm">{{ __('messages.nav_login') }}</a>
                <a href="{{ route('register') }}" class="px-4 py-2.5 bg-primary text-white rounded-xl font-semibold text-sm shadow-md hover:bg-primary-light transition-colors">{{ __('messages.nav_register') }}</a>
            @endauth
        @endif
    </div>
</nav>
...
<main class="max-w-6xl mx-auto px-4 sm:px-6 py-12 md:py-20 text-center">
    ...
    <h1 class="max-w-4xl mx-auto text-4xl md:text-6xl font-extrabold leading-tight mb-6 md:mb-8">
        {{ __('messages.welcome_heading') }} <br><span class="text-primary">{{ __('messages.welcome_heading_accent') }}</span>
    </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mt-20 md:mt-24 text-left">
        <div class="p-6 md:p-8 bg-white rounded-2xl border border-slate-200 shadow-sm">
            ...
            <h3 class="text-lg md:text-xl font-bold text-slate-800 mb-2">{{ __('messages.welcome_adaptive') }}</h3>
            <p class="text-slate-500 text-sm md:text-base">{{ __('messages.welcome_adaptive_desc') }}</p>
        </div>
        <div class="p-6 md:p-8 bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-2xl md:text-3xl mb-4">📝</div>
            <h3 class="text-lg md:text-xl font-bold text-slate-800 mb-2">{{ __('messages.welcome_quizzes') }}</h3>
            <p class="text-slate-500 text-sm md:text-base">{{ __('messages.welcome_quizzes_desc') }}</p>
        </div>
        <div class="p-6 md:p-8 bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-2xl md:text-3xl mb-4">💬</div>
            <h3 class="text-lg md:text-xl font-bold text-slate-800 mb-2">{{ __('messages.welcome_forum_title') }}</h3>
            <p class="text-slate-500 text-sm md:text-base">{{ __('messages.welcome_forum_desc') }}</p>
        </div>
    </div>
</main>
