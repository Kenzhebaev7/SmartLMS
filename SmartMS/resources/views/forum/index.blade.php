<x-app-layout>
    ...
    <div class="mb-4 p-4 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-sm text-slate-700 dark:text-slate-300">
        {{ __('messages.forum_projects_description') }}
    </div>

    <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
        <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ __('messages.forum_topics') }}</h2>
        <div class="flex flex-wrap items-center gap-3">
            <form action="{{ route('forum.index') }}" method="GET" class="inline-flex items-center gap-2">
                <label for="section_id" class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('messages.forum_by_section') }}</label>
            ...
            <a href="{{ route('forum.create') }}" class="px-6 py-3 bg-primary text-white rounded-xl font-semibold hover:bg-primary-light transition-colors">{{ __('messages.forum_new_topic') }}</a>
        </div>
    </div>
