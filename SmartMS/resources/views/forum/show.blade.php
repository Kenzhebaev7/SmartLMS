<x-app-layout>
    ...
    <div class="mb-6 p-6 rounded-xl border border-gray-200 bg-white">
        ...
        @auth
            @if(in_array(auth()->user()->role, [\App\Models\User::ROLE_TEACHER, \App\Models\User::ROLE_ADMIN]))
                <form action="{{ route('forum.threads.destroy', $thread) }}" method="POST" class="mt-4 inline" onsubmit="return confirm('{{ __('messages.forum_confirm_hide_topic') }}');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-sm text-amber-600 hover:text-amber-800 font-medium">{{ __('messages.forum_hide_topic') }}</button>
                </form>
            @endif
        @endauth
    </div>

    <h3 class="text-lg font-bold text-slate-800 mb-4">{{ __('messages.forum_comments') }}</h3>
    ...
    <div class="mt-6">
        <a href="{{ route('forum.index') }}" class="text-primary font-semibold hover:underline">← {{ __('messages.forum_back_list') }}</a>
    </div>
