<x-guest-layout>
    <div class="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 md:p-10">
        <div class="mb-8 text-center">
            <h2 class="text-2xl md:text-3xl font-bold text-slate-800 tracking-tight">{{ __('messages.auth_login_title') }}</h2>
            <p class="text-slate-500 mt-1 text-sm">{{ __('messages.auth_login_subtitle') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div>
                <div class="flex justify-between items-center mb-1.5">
                    <label for="password" class="text-sm font-semibold text-slate-700">{{ __('messages.auth_password') }}</label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-xs font-semibold text-accent hover:text-accent-dark">{{ __('messages.auth_forgot_password') }}</a>
                    @endif
                </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="remember_me" class="text-sm text-slate-600">{{ __('messages.auth_remember') }}</label>
            </div>
            ...
            <p class="text-center text-sm text-slate-500">
                {{ __('messages.auth_first_time') }} <a href="{{ route('register') }}" class="font-semibold text-primary hover:underline">{{ __('messages.auth_create_account') }}</a>
            </p>
        </form>
    </div>
</x-guest-layout>
