<x-app-layout>
    <x-slot name="header">{{ __('teacher.new_section_header') }}</x-slot>
    ...
    <form action="{{ route('teacher.sections.store') }}" method="POST" class="max-w-xl space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title_kk" value="{{ old('title_kk') }}" placeholder="Атауы (қазақша)" class="w-full rounded-lg border border-gray-300 px-4 py-2 mb-2">
            <textarea name="description_kk" rows="2" placeholder="Сипаттама (қазақша)" class="w-full rounded-lg border border-gray-300 px-4 py-2">{{ old('description_kk') }}</textarea>
        </div>
        <div>
            ...
            <select name="level" class="w-full rounded-lg border border-gray-300 px-4 py-2">
                <option value="">{{ __('teacher.level_all') }}</option>
