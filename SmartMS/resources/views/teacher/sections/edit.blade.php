<x-app-layout>
    ...
    <form action="{{ route('teacher.sections.update', $section) }}" method="POST" class="max-w-xl space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title_kk" value="{{ old('title_kk', $section->title_kk) }}" placeholder="{{ __('messages.teacher_placeholder_title_kk') }}" class="w-full rounded-lg border border-gray-300 px-4 py-2 mb-2">
            <textarea name="description_kk" rows="2" placeholder="{{ __('messages.teacher_placeholder_description_kk') }}" class="w-full rounded-lg border border-gray-300 px-4 py-2">{{ old('description_kk', $section->description_kk) }}</textarea>
        </div>
        <div>
            ...
            <select name="grade" class="w-full rounded-lg border border-gray-300 px-4 py-2">
                <option value="" {{ old('grade', $section->grade) === null ? 'selected' : '' }}>{{ __('messages.teacher_grade_all') }}</option>
                <option value="9" {{ old('grade', $section->grade) == 9 ? 'selected' : '' }}>{{ __('messages.auth_grade_9') }}</option>
                <option value="10" {{ old('grade', $section->grade) == 10 ? 'selected' : '' }}>{{ __('messages.auth_grade_10') }}</option>
                <option value="11" {{ old('grade', $section->grade) == 11 ? 'selected' : '' }}>{{ __('messages.auth_grade_11') }}</option>
            </select>
