        ...
        <table class="min-w-full border border-gray-200 rounded-xl overflow-hidden">
            <thead class="bg-gray-50 dark:bg-slate-700">
                <tr>
                    ...
                    @endforeach
                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('messages.sections_master_badge') }}</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('messages.teacher_achievements') }}</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-slate-800 dark:text-slate-100">{{ __('messages.teacher_feedback') }}</th>
                </tr>
            </thead>
                    ...
                    <tr class="border-t border-gray-200">
                            ...
                            @foreach($sections ?? [] as $s)
                                ...
                                @else
                                    <form action="{{ route('teacher.progress.master') }}" method="POST" class="inline">
                                        ...
                                        <button type="submit" class="text-primary text-sm hover:underline">{{ __('messages.teacher_assign_master', ['section' => $s->title]) }}</button>
                                    </form>
                                @endif
                            @endforeach
                        </td>
                        <td class="px-4 py-3">
                            @if(isset($achievements) && $achievements->isNotEmpty())
                                <form action="{{ route('teacher.students.achievements.award', $student) }}" method="POST" class="inline-flex gap-1 flex-wrap">
                                    ...
                                    <button type="submit" class="shrink-0 px-2 py-1 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">{{ __('messages.teacher_award_achievement') }}</button>
                                </form>
                            @else
                                —
                            @endif
                        ...
                        <td class="px-4 py-3">
                            <form action="{{ route('teacher.progress.feedback') }}" method="POST" class="flex gap-2">
                                ...
                                <textarea name="body" rows="2" placeholder="{{ __('messages.teacher_feedback_placeholder') }}" class="flex-1 min-w-[120px] rounded-lg border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 px-2 py-1 text-sm">{{ isset($feedbacks[$student->id]) ? $feedbacks[$student->id]->body : '' }}</textarea>
                                <button type="submit" class="shrink-0 px-3 py-1 bg-primary text-white rounded-lg text-sm hover:bg-primary-light">{{ __('messages.teacher_save_feedback') }}</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
