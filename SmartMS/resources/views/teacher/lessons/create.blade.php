<x-app-layout>
    ...
    <div class="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
        <form action="{{ route('teacher.sections.lessons.store', $section) }}" method="POST" enctype="multipart/form-data" class="space-y-6 rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm">
            @csrf

            <div class="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-4 text-sm text-sky-900">
                {{ __('messages.teacher_lesson_context_hint', ['grade' => $section->grade ? __('messages.auth_grade_' . $section->grade) : __('messages.teacher_grade_all'), 'level' => $section->is_revision ? __('messages.dashboard_level_beginner') : __('messages.dashboard_level_advanced')]) }}
            </div>
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="title_kk" value="{{ old('title_kk') }}" placeholder="{{ __('messages.teacher_placeholder_topic_kk') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ __('messages.teacher_order') }}</label>
                    <input type="number" name="order" value="{{ old('order', 0) }}" min="0" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800">
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ __('messages.teacher_content') }} (RU)</label>
                    <textarea name="content" rows="9" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800">{{ old('content') }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ __('messages.teacher_content') }} (KK)</label>
                    <textarea name="content_kk" rows="6" placeholder="{{ __('messages.teacher_placeholder_content_kk') }}" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-slate-800">{{ old('content_kk') }}</textarea>
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div class="mt-5 grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="video_url" value="{{ old('video_url') }}" placeholder="{{ __('messages.teacher_placeholder_video_url') }}" autocomplete="off" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800">
                    ...
                    <div>
                        ...
                        <input type="text" name="video_id" value="{{ old('video_id') }}" placeholder="{{ __('messages.teacher_placeholder_video_id') }}" autocomplete="off" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800">
                    ...
                    <div>
                        ...
                        <input type="text" name="video_url_kk" value="{{ old('video_url_kk') }}" placeholder="{{ __('messages.teacher_placeholder_video_url') }}" autocomplete="off" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800">
                    ...
                    <div>
                        ...
                        <input type="text" name="video_id_kk" value="{{ old('video_id_kk') }}" placeholder="{{ __('messages.teacher_placeholder_video_id') }}" autocomplete="off" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800">
                ...
                <div class="mt-4">
                    <select name="unlock_after_lesson_id" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800">
                        ...
                        @foreach($availableLessons ?? [] as $availableLesson)
                            <option value="{{ $availableLesson->id }}" @selected((string) old('unlock_after_lesson_id') === (string) $availableLesson->id)>
        ...
        <aside class="space-y-6 rounded-[30px] border border-slate-200 bg-gradient-to-br from-white to-amber-50/50 p-6 shadow-sm">
            <div>
                <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">{{ __('messages.teacher_lessons') }}</p>
                <h2 class="mt-2 text-2xl font-bold text-slate-900">{{ __('messages.teacher_lesson_builder_title') }}</h2>
                <p class="mt-2 text-sm leading-7 text-slate-600">{{ __('messages.teacher_lesson_builder_desc') }}</p>
            </div>

            <div class="space-y-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                    <p class="text-sm font-semibold text-slate-900">{{ __('messages.teacher_lesson_tip_structure') }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ __('messages.teacher_lesson_tip_structure_desc') }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                    <p class="text-sm font-semibold text-slate-900">{{ __('messages.teacher_lesson_tip_media') }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ __('messages.teacher_lesson_tip_media_desc') }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-4">
                    <p class="text-sm font-semibold text-slate-900">{{ __('messages.teacher_lesson_tip_unlock') }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ __('messages.teacher_lesson_tip_unlock_desc') }}</p>
                </div>
            </div>
        </aside>
    </div>
