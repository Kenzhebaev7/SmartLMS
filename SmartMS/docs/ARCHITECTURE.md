...
## 13. Рекомендации по доработке (что ещё можно добавить)

- **Онлайн-компилятор (IDE):** на странице урока в разделах по C++/Python выводится блок с iframe (OnlineGDB, Trinket) и ссылкой «Открыть в новой вкладке» на случай блокировки встраивания. При необходимости явно задать разделы с IDE: в `.env` указать `IDE_CPP_SECTION_IDS=1,2` и/или `IDE_PYTHON_SECTION_IDS=3` (ID разделов через запятую).
- **Двуязычность (каз/рус):** разделы и уроки поддерживают `title_kk`, `description_kk`, `content_kk`; во всех представлениях используется `getTitleForLocale(app()->getLocale())` и аналоги для описания и контента. Сидер `InformaticsByGradeSeeder` заполняет оба языка.
- **Дополнительные фичи для платформы:**
  - Уведомления: «Новый комментарий в теме», «Учитель назначил достижение», «Раздел разблокирован».
  - Экспорт прогресса: PDF/Excel отчёт для учителя по классу или для ученика — личный прогресс по разделам.
  - Таймер/дедлайны: ограничение по времени на прохождение квиза, даты сдачи разделов.
  - Офлайн-режим: PWA, кэширование конспектов уроков для просмотра без сети.
  - Поиск по материалам: полнотекстовый поиск по заголовкам и контенту уроков (каз/рус).
  - Статистика и графики: время в системе, динамика прохождения квизов, сравнение с группой (анонимно).
  - Мобильное приложение: обёртка через Capacitor/React Native или отдельный API для приложения.
  - Интеграция с электронным дневником/школьной системой (1С, Kundelik и т.п.) через API.
  - Голосовое озвучивание конспектов (TTS) для доступности.
  - Достижения за серию: «5 квизов подряд на отлично», «Первый комментарий в форуме».
  - Родительский доступ: просмотр прогресса ребёнка (только чтение) по приглашению или привязке аккаунта.
